...
- **Phase 9 — Teamleader-sync (tijd + PDF)**, zie `claude/phase9-teamleader-sync.md` in het Claude-project voor de volledige overdrachtsnotitie:
  ...
  - **`swatt-sync-worker` draait momenteel NIET op Render**: Background Worker-services worden niet ondersteund op het gratis plan (bevestigd via render.com/pricing) — de service uit `render.yaml` wordt daardoor nooit effectief aangemaakt. Als tijdelijke, gratis oplossing staat `RUN_SYNC_WORKER_INLINE=true` aan bij `swatt-api` (zie `apps/api/src/server.ts`/`config/env.ts`): de BullMQ-syncwerker draait daardoor mee in hetzelfde proces als de API, zodat de Teamleader-sync wél echt werkt voor demo's. **Zodra `swatt-sync-worker` naar een betaald Render-plan gaat:** zet `RUN_SYNC_WORKER_INLINE` terug op `false` (of verwijder de regel) bij `swatt-api` in `render.yaml`, en zet `plan: free` bij `swatt-sync-worker` op een betaald plan — anders draait de sync dubbel (onschadelijk door de idempotentie uit business rule 5, maar onnodig).
