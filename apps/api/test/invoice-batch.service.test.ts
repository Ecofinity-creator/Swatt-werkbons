  ...
  function hydrateBatch(id: string) {
    ...
    return {
      ...
      lines: batchLines.map((line) => {
        ...
        return { id: line.id, workOrderId: line.workOrderId, invoiceableSeconds: line.invoiceableSeconds, workOrder: { workOrderNumber: wo?.workOrderNumber ?? '?', project: { name: wo?.project.name ?? '?' } } };
      }),
    };
  }
...
describe('InvoiceBatchService', () => {
  it('listInvoiceable() toont enkel READY_FOR_INVOICING werkbonnen die nog niet gebatcht zijn, met correct berekende uren', async () => {
    const wo1 = workOrder({
      id: 'wo1',
      timeEntries: [{ timeEntry: { startedAt: new Date('2026-08-10T08:00:00Z'), endedAt: new Date('2026-08-10T10:00:00Z'), pausedSeconds: 0, employeeId: peter, employee: { displayName: 'Peter Janssens' } } }],
    });
  ...
  it('listInvoiceable() filtert op periodLabel (ondertekeningsmaand) en op werknemer', async () => {
    ...
    const wo2 = workOrder({
      ...
      timeEntries: [{ timeEntry: { startedAt: new Date('2026-07-15T08:00:00Z'), endedAt: new Date('2026-07-15T09:30:00Z'), pausedSeconds: 0, employeeId: wannes, employee: { displayName: 'Wannes' } } }],
    });
  ...
  it('create() maakt een batch aan, telt de uren correct op en maakt de werkbon meteen onbeschikbaar voor een volgende batch', async () => {
    const wo1 = workOrder({
      id: 'wo1',
      timeEntries: [{ timeEntry: { startedAt: new Date('2026-08-10T08:00:00Z'), endedAt: new Date('2026-08-10T10:17:00Z'), pausedSeconds: 17 * 60, employeeId: peter, employee: { displayName: 'Peter Janssens' } } }],
    });
  ...
  it('remove() verwijdert een DRAFT-batch volledig, en geeft de werkbon weer vrij', async () => {
    const wo1 = workOrder({ id: 'wo1', timeEntries: [{ timeEntry: { startedAt: new Date('2026-08-10T08:00:00Z'), endedAt: new Date('2026-08-10T09:00:00Z'), pausedSeconds: 0, employeeId: peter, employee: { displayName: 'Peter Janssens' } } }] });
    const { prisma } = createFakePrisma([wo1]);
    const service = new InvoiceBatchService(prisma);
    const batch = await service.create({ customerId: janssens.id, periodLabel: '2026-08', workOrderIds: ['wo1'], createdByUserId: 'user-admin' });
